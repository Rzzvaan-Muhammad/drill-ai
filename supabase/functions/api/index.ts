...
serve(async (req) => {
  ...
  if (req.method==="POST" && url.pathname==="/upload/complete") {
    const body = await req.json(); // { wellId, rows } rows = [{depth, composition, DT, GR}]
    const rows = body.rows.map((r:any)=>({well_id:body.wellId, depth:r.depth, composition:r.composition, dt:r.DT, gr:r.GR}));
  ...
  const mTracks = url.pathname.match(/^\/wells\/(.+)\/tracks$/);
  ...
  if (req.method==="POST" && url.pathname==="/chat") {
    ...
    const { data } = await supabase.from("tracks").select("depth,composition,dt,gr").eq("well_id", wellId).order("depth").limit(1200);
